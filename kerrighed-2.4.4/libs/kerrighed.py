...
class krg_error_handler(object):
    ...
    def __call__(self, value):
        if value==-1:
            raise kerrighed_error("error in %s: %s" % (self.func.__name__,
                                                       os.strerror(libkerrighed.krg_get_errno())))
        else:
            return value
...
libkerrighed.krg_nodes_init.restype = krg_nodes_ptr_t
...
libkerrighed.krg_clusters_init.restype = krg_clusters_ptr_t
libkerrighed.krg_clusters_is_up.restype = krg_error_handler(libkerrighed.krg_clusters_is_up)

libkerrighed.krg_node_set_init.restype = krg_node_set_ptr_t
...
libkerrighed.krg_get_max_nodes.restype = krg_error_handler(libkerrighed.krg_get_max_nodes)
libkerrighed.krg_get_max_clusters.restype = krg_error_handler(libkerrighed.krg_get_max_clusters)
...
class krg_nodes(object):
    ...
    def __init__(self, _c=None):
        self.cur = -1
        if not _c:
            _c = libkerrighed.krg_nodes_init()
            if not _c:
                raise krg_error_handler("krg_nodes_init returned NULL")
        self.c = _c
...
class krg_clusters(object):
    ...
    def __init__(self, _c=None):
        if not _c:
            _c = libkerrighed.krg_clusters_init()
            if not _c:
                raise krg_error_handler("krg_clusters_init returned NULL")
        self.c = _c
...
class krg_node_set(object):
    ...
    def __init__(self, _c=None):
        ...
        self.pop_cur = -1
        if not _c:
            _c = libkerrighed.krg_node_set_init()
            if not _c:
                raise krg_error_handler("krg_node_set_init returned NULL")
        self.c = _c
...
def krg_get_max_nodes():
    """
    return: int
    """
    return libkerrighed.krg_get_max_nodes()

def krg_get_max_clusters():
    """
    return: int
    """
    return libkerrighed.krg_get_max_clusters()
